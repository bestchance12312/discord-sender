...
def send_message(conn, channel_id, message_data, header_data):
    try:
        ...
        if 199 < resp.status < 300:
            print(f"{get_timestamp()} Messages sent with status {resp.status}!")
        else:
            print(f"{get_timestamp()} Failed to send messages with status {resp.status}.")
    except Exception as e:
        print(f"{get_timestamp()} Error sending messages: {e} | {message_data}")
...
def main():
    ...
    if not info or len(info) != 4:
        print(
            ...
            f"the following information in order: User ID, Discord token, Discord channel URL, and Discord channel "
    ...
    while True:
        ...
        if not messages:
            print(f"{get_timestamp()} No messages to send.")
            return

        # Combine all messages into a single payload
        combined_message = "\n".join(messages)
        message_data = json.dumps({"content": combined_message})

        print(f"{get_timestamp()} Sending combined message:\n{combined_message}")

        conn = get_connection()
        send_message(conn, info[3], message_data, header_data)
        conn.close()

        print(f"{get_timestamp()} Finished sending all messages in this cycle!")
        random_sleep(delay_between_messages, 1, 10)  # Sleep between sending each batch of combined messages

        print(f"{get_timestamp()} Sleeping for {sleep_time} seconds before next cycle...")
